...
PAYMENT_HOST = os.getenv("PAYMENT_HOST", "http://payment-service:5002")
...
async def list_payments_proxy(request: Request):
    upstream_url = f"{PAYMENT_HOST}/api/v1/payments"
...
async def confirm_payment_proxy(order_id: str, request: Request):
    upstream_url = f"{PAYMENT_HOST}/api/v1/payments/confirm/{order_id}"
    return await proxy_request(request, upstream_url)
